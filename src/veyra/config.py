...
class ModelConfig(BaseModel):
    ...
    backend: str = Field(
        default="mock", description="Model backend: mock, openai, anthropic"
    )
...
class LatencyConfig(BaseModel):
    ...
    simulate_latency: bool = Field(
        default=False, description="Enable latency simulation"
    )
    min_delay_seconds: float = Field(
        default=180.0, description="Minimum delay (Mars closest: 3 min)"
    )
    max_delay_seconds: float = Field(
        default=1320.0, description="Maximum delay (Mars farthest: 22 min)"
    )
...
class GovernanceConfig(BaseModel):
    ...
    safety_boundaries: bool = Field(
        default=True, description="Enable hard safety boundaries"
    )
    reversible_only: bool = Field(
        default=False, description="Only allow reversible operations"
    )
...
class VeyraConfig(BaseModel):
    ...
    world_model_enabled: bool = Field(
        default=False, description="Enable world model reasoning"
    )
    environment: str = Field(
        default="earth", description="Target environment: earth, mars, lunar, space"
    )
